    ...
    const connectors: Connection[] = [
      {
        ...
        toolsCount: allTools.filter(t => t.name.startsWith('memory_') || t.name.startsWith('artifact_') || t.name.startsWith('link_') || t.name.startsWith('run_')).length,
        latency: 5,
      },
      {
        id: 'conn-gamma',
      ...
      },
      {
        id: 'conn-confluence',
        name: 'Confluence',
        type: 'Atlassian Connector',
        endpoint: 'Atlassian API',
        enabled: true,
        status: 'healthy',
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        toolsCount: allTools.filter(t => t.name.startsWith('confluence_')).length,
        latency: 150,
      },
      {
        id: 'conn-github',
        ...
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        toolsCount: allTools.filter(t => t.name.startsWith('github_')).length,
        latency: 200,
      },
      {
        id: 'conn-slack',
